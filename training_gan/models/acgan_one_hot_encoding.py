...
def build_generator(noise_dim, label_dim):
    """
    Builds the generator for the ACGAN using one-hot encoding labels as
    conditioning.
    ...
    input: label_dim: dimension of the one-hot encodings which is 20.
    return: a model that takes noise and OHE labels as input and outputs a
            64x63x3 image.
    """

    noise_input = keras.layers.Input(shape=(noise_dim,))
    label_input = keras.layers.Input(shape=(label_dim,))

    concat = Concatenate()([noise_input, label_input])
    ...
    return Model(inputs=[noise_input, label_input], outputs=[output])


def build_discriminator(input_img_shape):
    """
    Builds the discriminator for the ACGAN using one-hot encoding labels as
    conditioning.
...
def build_gan(noise_dim, label_dim, gen_model, dis_model):
    """
    ...
    noise_input = keras.layers.Input(shape=(noise_dim,))
    label_input = keras.layers.Input(shape=(label_dim,))
    g_output = gen_model(inputs=[noise_input, label_input])
    dis_model.trainable = False
    fake, aux = dis_model(inputs=[g_output])
    return Model(inputs=[noise_input, label_input], outputs=[fake, aux])